...
\begin{itemize}
    \item \textbf{Client-Server}: A \textit{homeserver} interacting with a \textit{client}. In \ac{P2P} Matrix, the \textit{client} and \textit{homeserver} are both on-device.
...
\begin{itemize}
    ...
    \item{
          ...
          Using Gomobile, Dendrite can be ported to mobile, e.g. as a library in the client app, effectively embedding the server in the mobile client.
          }
    \item{
          \textbf{Efficiency}
...
\begin{figure}
    \centering
    \resizebox{0.8\linewidth}{!}{\input{graphics/p2p-embed.tex}}
    \caption{
