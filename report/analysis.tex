...
The homeserver can forward events to the Push Gateway, which then passes this information to push notification services provided by Google and Apple.
As of the writing of this report, this API is trivial, with only one defined request.
It is also unnecessary for a homeserver to contact a service external to the device in order to provide a notification.
Ideally, this notification could be displayed directly by the homeserver, or by calling a custom Push Gateway which avoids the extra round trip.
